...
# --- GENERADOR DE DATOS DEMO (JUNIO Y JULIO) ---
def cargar_datos_demo():
    for mes_num, dias_mes in [("06", 30), ("07", 15)]:
        for _ in range(30): # 30 lotes por mes
            ...
            add_data("produccion_final", (
                prod, str(random.randint(1, 50)), "Bandeja Estándar", 
                fecha_sim.strftime("%Y-%m-%d"), fecha_luz.strftime("%Y-%m-%d"), estado_luz, 
                fecha_cos.strftime("%Y-%m-%d"), estado_cos, 
                random.uniform(30, 50), tapers, "Taper 12onz", 0.0, merma, "Microgreens"
            ))

# --- MOTOR DE IA CONSULTORA ---
def analizar_datos_gea_avanzado(df, pregunta):
    if df.empty: return "No hay datos suficientes en este período para analizar."
    ...
    # 1. Si preguntan por Pareto, Diagrama, Opinión o Eficiencia
    if any(word in p for word in ['pareto', 'diagrama', 'opinion', 'opinión', 'eficiencia', 'mejora', 'mejorar']):
        ...
        if cultivos_80:
            response += f"El análisis de Pareto revela que el 80% de tus pérdidas se concentran en: **{', '.join(cultivos_80)}**. Esto significa que no tienes un problema generalizado, sino focos de ineficiencia específicos.\n\n"
        else:
            response += "Las mermas están distribuidas uniformemente, no hay un cultivo crítico dominante.\n\n"
            
        response += "**2. Análisis de Causa Raíz:**\n"
        df_def = df[df['estado_cosecha'] == '✗ Deficiente']
        if not df_def.empty:
            response += f"Los lotes en estado 'Deficiente' están generando una merma promedio de {df_def['merma'].mean():.1f}g, muy por encima del promedio general. Esto suele estar correlacionado con dos factores: dosificación imprecisa de sustrato (al usar volumen en lugar de peso) y fallas en el control de humedad durante la transición a la luz.\n\n"
            
        response += "**3. Plan de Acción para Mejorar la Eficiencia:**\n"
        response += f"- **Acción 1 (Inmediata):** Estandarizar el pesaje en gramos del sustrato para '{cultivo_top_merma}'.\n"
        response += "- **Acción 2 (Proceso):** Implementar alertas visuales en el área de germinación para los lotes que pasan a luz en estado 'Regular'.\n"
        response += "- **Acción 3 (Calidad):** Replicar el protocolo exacto de los lotes 'Óptimos' (✔) hacia los cultivos con menor rendimiento.\n"
        return response
        
    # 2. Respuesta por defecto (Resumen General)
    else:
        response += f"**1. Resumen Operativo:**\nSe procesaron {len(df)} lotes, generando {total_tapers} tapers. La merma total es {total_merma:.1f}g, con un ratio de pérdida del {ratio_merma:.1f}%.\n\n"
        response += f"**2. Punto de Atención:**\nEl cultivo que requiere atención inmediata es **{cultivo_top_merma}**.\n\n"
        response += "**3. Recomendación:**\nUtiliza el Diagrama de Pareto en el panel principal para identificar qué cultivos generar el 80% de tus pérdidas y enfocar ahí los esfuerzos de mejora."
        return response
        ...
        if user['area'] == "Microgreens":
            ...
            with st.expander("📄 Descargar Plantilla Oficial de Excel"):
                ...
                df_plantilla = pd.DataFrame(columns=[
                    'Identificación de Producto', 'Numero de bandeja/Lote', 'Tipo de bandeja', 'Fecha de siembra', 
                    'Fecha de transición a la luz', 'Estado del cultivo de transicion a la luz', 'Fecha de cosecha', 
                    'Estado del cultivo en cosecha', 'Peso bruto', 'Cantidad de tapers', 'Taper usado para envasado', 
                ...
                file_trab = st.file_uploader("Subir Excel", type=['xlsx'], key="excel_trab")
                if file_trab is not None:
                    try:
                        ...
                        for index, row in df_excel.iterrows():
                            ...
                            add_data("produccion_final", (
                                prod, str(row.get('Numero de bandeja/Lote', '')).strip(), str(row.get('Tipo de bandeja', '')).strip(),
                                parse_date(row.get('Fecha de siembra')), parse_date(row.get('Fecha de transición a la luz')), str(row.get('Estado del cultivo de transicion a la luz', '✔ Óptimo')).strip(),
                                parse_date(row.get('Fecha de cosecha')), str(row.get('Estado del cultivo en cosecha', '✔ Óptimo')).strip(),
                                float(row.get('Peso bruto', 0) or 0), int(row.get('Cantidad de tapers', 0) or 0), str(row.get('Taper usado para envasado', '')).strip(),
                                float(row.get('Peso sobrante/ Remanente', 0) or 0), float(row.get('Merma', 0) or 0), "Microgreens"
                            ))
                        st.success(f"¡Éxito! Se cargaron {len(df_excel)} registros desde tu Excel.")
                    except Exception as e:
                        st.error(f"Error al leer el archivo. Asegúrate de usar la plantilla. {e}")
            ...
            with st.form("form_cosecha_v3"):
                ...
                with c1:
                    producto = st.selectbox("Identificación de Producto", PRODUCTOS)
                    lote = st.text_input("Numero de bandeja/Lote", "1")
                    tipo_bandeja = st.selectbox("Tipo de bandeja", BANDEJAS)
                with c2:
                    fecha_siembra = st.date_input("Fecha de siembra", date.today())
                    fecha_luz = st.date_input("Fecha de transición a la luz", date.today())
                    estado_luz = st.selectbox("Estado transición a la luz", ESTADOS)
                with c3:
                    fecha_cosecha = st.date_input("Fecha de cosecha", date.today())
                    estado_cosecha = st.selectbox("Estado en cosecha", ESTADOS)
                    peso_bruto = st.number_input("Peso bruto (g)", min_value=0.0, step=0.1)
                c4, c5 = st.columns(2)
                with c4:
                    cantidad_tapers = st.number_input("Cantidad de tapers", min_value=0, step=1)
                    taper_envasado = st.selectbox("Taper usado para envasado", TAPERS)
                with c5:
                    peso_sobrante = st.number_input("Peso sobrante/Remanente (g)", min_value=0.0, step=0.1)
                    merma = st.number_input("Merma (g)", min_value=0.0, step=0.1)
            ...
            with st.form("form_siembra"):
                c1, c2 = st.columns(2)
                with c1:
                    cultivo = st.selectbox("Cultivo", PRODUCTOS)
                    bandejas = st.number_input("N° Bandejas", min_value=1, step=1)
                with c2:
                    semilla = st.number_input("Semilla usada (g)", min_value=1.0, step=1.0)
                    fecha = st.date_input("Fecha de Siembra", date.today())
                if st.form_submit_button("Registrar Siembra"):
                    add_data("siembra_v2", (cultivo, bandejas, "Por definir", semilla, fecha.strftime("%Y-%m-%d")))
                    st.success("Siembra registrada. Lote en germinación.")
            ...
            with tab_v:
                with st.form("form_ventas"):
                    ...
                    if st.form_submit_button("Registrar Venta"):
                        add_data("ventas_v2", (prod, cant, precio, date.today().strftime("%Y-%m-%d")))
            ...
            with tab_p:
                with st.form("form_perdidas"):
                    ...
                    if st.form_submit_button("Registrar Pérdida"):
                        add_data("perdidas_v2", (prod, cant, motivo, date.today().strftime("%Y-%m-%d")))
            ...
            with tab_c:
                with st.form("form_contactos"):
                    ...
                    if st.form_submit_button("Guardar Contacto"):
                        add_data("contactos_v2", (nombre, tel, email, pot))
                        st.success("Contacto guardado en CRM.")
    ...
    # --- VISTA JEFE ---
        ...
        if menu == "📈 Producción (Tiempo Real)":
            ...
            if uploaded_file is not None:
                try:
                    ...
                    for index, row in df_excel.iterrows():
                        ...
                        add_data("produccion_final", (
                            prod, str(row.get('Numero de bandeja/Lote', '')).strip(), str(row.get('Tipo de bandeja', '')).strip(),
                            parse_date(row.get('Fecha de siembra')), parse_date(row.get('Fecha de transición a la luz')), map_state(row.get('Estado del cultivo de transicion a la luz')),
                            parse_date(row.get('Fecha de cosecha')), map_state(row.get('Estado del cultivo en cosecha')),
                            safe_num(row.get('Peso bruto')), safe_num(row.get('Cantidad de tapers'), is_int=True), str(row.get('Taper usado para envasado', '')).strip(),
                            safe_num(row.get('Peso sobrante/ Remanente')), merma, "Microgreens"
                        ))
            ...
            if df.empty:
                ...
                if st.button("🎲 Generar Datos de Demostración (Junio y Julio)"):
                    cargar_datos_demo()
                    st.rerun()
            else:
                # --- CARPETAS MENSUALES ---
                ...
                with col_filtro:
                    mes_sel_txt = st.selectbox("Selecciona el mes a analizar:", ["Ver Todo el Año"] + meses_formateados)
                with col_demo:
                    st.write("") # Espaciador
                    if st.button("🎲 Cargar Demo Jun/Jul"):
                        cargar_datos_demo()
                        st.rerun()

                if mes_sel_txt != "Ver Todo el Año":
                    mes_sel_num = mapa_meses[mes_sel_txt]
                    df = df[df['mes_num'] == mes_sel_num]
                    st.write(f"Mostrando datos de: **{mes_sel_txt}**")
                else:
                    st.write("Mostrando datos de: **Todo el Año**")
                ...
                if st.button("Generar Informe IA"):
                    if pregunta:
                        ...
                        respuesta_ia = analizar_datos_gea_avanzado(df, pregunta)
                        st.session_state.chat_history.append({"role": "bot", "content": respuesta_ia})
                        st.rerun()
            ...
            if df_prod.empty:
                ...
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    df_export.to_excel(writer, index=False, sheet_name='Hoja2')
                ...
                if st.button("🤖 GENERAR REPORTE EJECUTIVO AHORA", use_container_width=True, type="primary"):
                    total_lotes = len(df_prod)
                    total_tapers = df_prod['cantidad_tapers'].sum()
                    total_merma = df_prod['merma'].sum()
                    ratio_merma = (total_merma / total_tapers) * 100 if total_tapers > 0 else 0
...
=======================================================
1. RESUMEN GENERAL:
Hasta la fecha, la planta ha procesado {total_lotes} lotes, logrando producir {total_tapers} tapers. 
La merma acumulada es de {total_merma:.1f} gramos (ratio del {ratio_merma:.1f}%). 
Estado general: {estado_general}.

2. DEFICIENCIAS:
- {num_deficientes} lotes deficientes (pérdida de biomasa).
- Cultivo crítico: '{cultivo_top_merma}'.

3. MEJORAS:
- Estandarizar peso de sustrato para {cultivo_top_merma}.
- Replicar protocolo de lotes 'Óptimos' en áreas de menor rendimiento.
=======================================================
"""
            ...
            if st.button("Actualizar Código Maestro"):
                if len(nuevo_pin) >= 6:
                    set_config('pin_jefe', nuevo_pin)
                    st.success("Código actualizado con éxito.")
                else:
                    st.error("El código debe tener al menos 6 caracteres.")
